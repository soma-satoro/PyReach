...
class CmdTestXP(Command):
    ...
    def func(self):
        ...
        try:
            ...
            output.append(f"OK Experience handler accessible via character.experience")
            ...
            if hasattr(self.caller, 'db') and self.caller.db.stats:
                ...
                if 'beats' in other_stats or 'experience' in other_stats:
                    ...
                    output.append(f"OK No legacy beats/XP in db.stats")
            
            # Test 3: Fractional beats
            fractional = self.caller.attributes.get('fractional_beats', default=0.0)
            output.append(f"OK Fractional beats system: {fractional}")
            ...
            output.append(f"OK Added 0.3 fractional beats: {old_total} -> {new_total}")
            
            # Test 5: Check spending system access
            stats = self.caller.db.stats
            if stats:
                output.append(f"OK Character stats accessible for spending")
            ...
            output.append(f"OK System mode check:")
            ...
            output.append(f"OK Dice roll integration ready")
